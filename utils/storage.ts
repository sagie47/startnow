...
export interface ReplanHistory {
  date: string;
  mode: 'keep-priorities' | 'salvage-streak' | 'hard-reset';
  driftMinutes: number;
  skippedCount: number;
  shrunkCount: number;
}

const CHAT_HISTORY_KEY = '@rocket_chat_history';
const REPLAN_HISTORY_KEY = '@rocket_replan_history';

export async function loadChatHistory(date: string): Promise<ChatHistory | null> {
  try {
    const data = await AsyncStorage.getItem(`${CHAT_HISTORY_KEY}_${date}`);
...
export async function saveChatHistory(history: ChatHistory): Promise<void> {
  try {
    await AsyncStorage.setItem(`${CHAT_HISTORY_KEY}_${history.date}`, JSON.stringify(history));
  } catch (error) {
    console.error('Failed to save chat history:', error);
  }
}

export async function loadReplanHistory(date: string): Promise<ReplanHistory | null> {
  try {
    const data = await AsyncStorage.getItem(`${REPLAN_HISTORY_KEY}_${date}`);
    return data ? JSON.parse(data) : null;
  } catch (error) {
    console.error('Failed to load replan history:', error);
    return null;
  }
}

export async function saveReplanHistory(history: ReplanHistory): Promise<void> {
  try {
    await AsyncStorage.setItem(`${REPLAN_HISTORY_KEY}_${history.date}`, JSON.stringify(history));
  } catch (error) {
    console.error('Failed to save replan history:', error);
    return null;
  }
}
